...
use onnx_ir::node::lstm::LstmConfig;
...
pub struct LstmNode {
    ...
    pub recurrent_weight: TensorData,
    pub bias: Option<TensorData>,
    pub sequence_lengths: Option<TensorData>,
...
impl LstmNode {
    pub fn new<S: AsRef<str>>(
        ...
        input: TensorType,
        weight: TensorData,
        recurrent_weight: TensorData,
        bias: Option<TensorData>,
        sequence_lengths: Option<TensorData>,
...
impl LstmNode {
    ...
    fn serialize_lstm_record<PS: PrecisionSettings>(
        ...
        let r_tensor: Tensor<_, 2> = Tensor::<_, 3, _>::from_data(
            self.recurrent_weight.clone().convert::<PS::FloatElem>(),
            device,
        )
        .slice(s![direction, ..])
        .squeeze(0);
...
impl<PS: PrecisionSettings> NodeCodegen<PS> for LstmNode {
    ...
    fn forward(&self, scope: &mut Scope, node_position: usize) -> TokenStream {
        ...
        let d_hidden = self.config.hidden_size;
    ...
    fn field_init(&self) -> Option<TokenStream> {
        ...
        let d_input = self.config.input_size;
        let d_hidden = self.config.hidden_size;
...
mod tests {
    ...
    fn test_codegen_forward() {
        ...
        graph.register(LstmNode::new(
            ...
            None,
            None,
            LstmConfig { input_size: 20, hidden_size: 128 }
        ));
